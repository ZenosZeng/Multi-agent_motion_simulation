...
def run_motion_simulation(sim_name,sim_time):
    ...
    omega = 0.5
    po2_d = [bian*sin(omega*t),-bian*cos(omega*t),0]
    po2_d_dot = [bian*omega*cos(omega*t),bian*omega*sin(omega*t),0]
    po2_bar = po2-po2_d  
    ...
    for step in range(total_step):
        ...
        pt = [ -5*cos(t)+5,5*sin(t)+10,2*t+8 ]
        dpt = [ 5*sin(t),5*cos(t),2]
        ...
        beta = 16 #  gain for signal term
        alpha = 40 # gain for orientation

        lamda = 10 # tracking gain 1
        ...
        if step%(interval)==0:
            ...
            target_trajectory[2].append(pt[2])
